...
var fetch = (0, import_ofetch.createFetch)({
  defaults: {
    onResponseError: (ctx) => {
...
var CwsApi = class {
  ...
  async uploadZip(params) {
    ...
    form.append("image", await (0, import_file_from_path.fileFromPath)(params.zipFile));
...
async function ensureZipExists(path) {
  try {
    await import_promises.default.lstat(path);
  } catch {
...
// src/chrome/chrome-web-store.ts
var ChromeWebStoreOptions = import_zod.z.object({
  zip: import_zod.z.string().min(1),
  extensionId: import_zod.z.string().min(1),
  clientId: import_zod.z.string().min(1),
  clientSecret: import_zod.z.string().min(1),
  refreshToken: import_zod.z.string().min(1),
  publishTarget: import_zod.z.enum(["default", "trustedTesters"]).default("default"),
  skipSubmitReview: import_zod.z.boolean().default(false)
});
var ChromeWebStore = class {
...
var EdgeApi = class {
  ...
  async uploadDraft(params) {
    ...
    const file = import_fs2.default.createReadStream(params.zipFile);
...
var EdgeAddonStoreOptions = import_zod2.z.object({
  zip: import_zod2.z.string().min(1),
  productId: import_zod2.z.string().min(1),
  clientId: import_zod2.z.string().min(1),
  clientSecret: import_zod2.z.string().min(1),
  accessTokenUrl: import_zod2.z.string().min(1),
  skipSubmitReview: import_zod2.z.boolean().default(false)
});
var EdgeAddonStore = class {
  constructor(options, setStatus) {
    this.options = options;
    this.setStatus = setStatus;
  ...
  async uploadAndPollValidation(token, pollIntervalMs) {
    this.setStatus("Uploading new ZIP file");
...
// src/firefox/firefox-api.ts
var import_formdata_node2 = require("formdata-node");
var import_file_from_path2 = require("formdata-node/file-from-path");
var import_jsonwebtoken = __toESM(require_jsonwebtoken(), 1);
var AddonsApi = class {
  constructor(options) {
    this.options = options;
  }
  addonDetailEndpoint(extensionId) {
    return new URL(
      `https://addons.mozilla.org/api/v5/addons/addon/${extensionId}`
    );
  }
  addonsUploadCreateEndpoint() {
    return new URL(`https://addons.mozilla.org/api/v5/addons/upload/`);
  }
  addonsUploadDetailsEndpoint(uploadUuid) {
    return new URL(
  ...
  async uploadCreate(params) {
    ...
    const form = new import_formdata_node2.FormData();
    form.append("channel", params.channel);
    form.append("upload", await (0, import_file_from_path2.fileFromPath)(params.file));
  ...
  async versionCreate(params) {
    ...
    if (params.sourceFile) {
      form.append("source", await (0, import_file_from_path2.fileFromPath)(params.sourceFile));
    } else {
      form.append("source", "");
    }
    return await fetch(endpoint.href, {
  ...
  createJwt(timeoutInS = 30) {
    ...
    return import_jsonwebtoken.default.sign(payload, secret, { algorithm: "HS256" });
  }
  getAuthHeader() {
...
var FirefoxAddonStoreOptions = import_zod3.z.object({
  zip: import_zod3.z.string().min(1),
  sourcesZip: import_zod3.z.string().min(1).optional(),
  extensionId: import_zod3.z.string().min(1),
  jwtIssuer: import_zod3.z.string().min(1),
  jwtSecret: import_zod3.z.string().min(1),
  channel: import_zod3.z.enum(["listed", "unlisted"]).default("listed")
});
var FirefoxAddonStore = class {
  constructor(options, setStatus) {
    this.options = options;
    this.setStatus = setStatus;
  ...
  async submit(dryRun) {
    ...
    this.setStatus("Submitting new version");
    const version2 = await this.api.versionCreate({
      extensionId: this.wrappedExtensionId,
      sourceFile: this.options.sourcesZip,
      uploadUuid: upload.uuid
    });
    const validationUrl = `https://addons.mozilla.org/en-US/developers/addon/${addon.id}/file/${version2.file.id}/validation`;
    ...
    if (!upload.valid)
      throw Error(`Extension is invalid: ${validationUrl}`);
    else
      console.log("Firefox validation results: " + validationUrl);
  }
  async uploadAndPollValidation(pollIntervalMs) {
    this.setStatus("Uploading new ZIP file");
...
function resolveConfig(config2) {
  const dryRun = config2.dryRun ?? booleanEnv("DRY_RUN") ?? false;
  const chromeZip = config2.chrome?.zip ?? stringEnv("CHROME_ZIP");
  const firefoxZip = config2.firefox?.zip ?? stringEnv("FIREFOX_ZIP");
  const edgeZip = config2.edge?.zip ?? stringEnv("EDGE_ZIP");
  return {
    dryRun,
    chrome: chromeZip == null ? void 0 : {
      zip: chromeZip,
      extensionId: config2.chrome?.extensionId ?? stringEnv("CHROME_EXTENSION_ID"),
      clientId: config2.chrome?.clientId ?? stringEnv("CHROME_CLIENT_ID"),
      clientSecret: config2.chrome?.clientSecret ?? stringEnv("CHROME_CLIENT_SECRET"),
      refreshToken: config2.chrome?.refreshToken ?? stringEnv("CHROME_REFRESH_TOKEN"),
      publishTarget: config2.chrome?.publishTarget ?? stringEnv("CHROME_PUBLISH_TARGET") ?? "default",
      skipSubmitReview: config2.chrome?.skipSubmitReview ?? booleanEnv("CHROME_SKIP_SUBMIT_REVIEW") ?? false
    },
    firefox: firefoxZip == null ? void 0 : {
      zip: firefoxZip,
      sourcesZip: config2.firefox?.sourcesZip ?? stringEnv("FIREFOX_SOURCES_ZIP"),
      extensionId: config2.firefox?.extensionId ?? stringEnv("FIREFOX_EXTENSION_ID"),
      jwtIssuer: config2.firefox?.jwtIssuer ?? stringEnv("FIREFOX_JWT_ISSUER"),
      jwtSecret: config2.firefox?.jwtSecret ?? stringEnv("FIREFOX_JWT_SECRET"),
      channel: config2.firefox?.channel ?? stringEnv("FIREFOX_CHANNEL") ?? "listed"
    },
    edge: edgeZip == null ? void 0 : {
      zip: edgeZip,
      productId: config2.edge?.productId ?? stringEnv("EDGE_PRODUCT_ID"),
      clientId: config2.edge?.clientId ?? stringEnv("EDGE_CLIENT_ID"),
      clientSecret: config2.edge?.clientSecret ?? stringEnv("EDGE_CLIENT_SECRET"),
      accessTokenUrl: config2.edge?.accessTokenUrl ?? stringEnv("EDGE_ACCESS_TOKEN_URL"),
      skipSubmitReview: config2.edge?.skipSubmitReview ?? booleanEnv("EDGE_SKIP_SUBMIT_REVIEW") ?? false
    }
  };
}
function validateConfig(config2) {
  const result = InternalConfig.safeParse(config2);
  if (!result.success) {
...
function booleanEnv(name) {
  return !process.env[name] ? void 0 : process.env[name] === "true";
}
function stringEnv(name) {
  return !process.env[name] ? void 0 : process.env[name];
}
var InlineConfig = import_zod4.z.object({
  /**
   * When true, just check authentication, don't upload any zip files or submit any updates.
   */
  dryRun: import_zod4.z.boolean().optional(),
  /**
...
var InternalConfig = import_zod4.z.object({
  dryRun: import_zod4.z.boolean(),
...
async function submit(config2) {
  const internalConfig = validateConfig(resolveConfig(config2));
  console.log("");
  import_consola.default.info("Publishing Extension");
  if (internalConfig.dryRun) {
    import_consola.default.warn("Dry run, skipping submission");
  }
  const stores = [];
  if (internalConfig.chrome) {
    stores.push({
      ...
      getStore: (setStatus) => new ChromeWebStore(internalConfig.chrome, setStatus)
    });
  }
  if (internalConfig.firefox) {
    stores.push({
      ...
      getStore: (setStatus) => new FirefoxAddonStore(internalConfig.firefox, setStatus)
    });
  }
  if (internalConfig.edge) {
    stores.push({
      ...
      getStore: (setStatus) => new EdgeAddonStore(internalConfig.edge, setStatus)
    });
  }
  if (stores.length === 0) {
...
var envFile = ".env.submit";
async function init(config2) {
  import_consola2.default.info(`Initialize or update an existing \`${envFile}\` file.`);
  const previousConfig = resolveConfig(config2);
  const stores = await prompt(
...
async function prompt(message, options, previousValue) {
  const result = await import_consola2.default.prompt(message, {
...
async function initChrome(previousOptions) {
  const entries = [];
  console.log();
  import_consola2.default.start("Chrome Web Store\n");
  import_consola2.default.log("`--chrome-extension-id` can be found:");
  import_consola2.default.log("  1. Under the extension name in the CWS developer console");
  import_consola2.default.log("  2. In the URL of the CWS page for the item");
  import_consola2.default.log("Example: `ocfdgncpifmegplaglcnglhioflaimkd`");
  ...
  const generateRefreshToken = await prompt(
  ...
  if (generateRefreshToken) {
    ...
    import_consola2.default.log(authCodeUrl);
    const authCode = await import_consola2.default.prompt(
    ...
    const res = await (0, import_ofetch2.ofetch)(tokenUrl, {
      method: "POST",
      body: data
    });
    const refreshToken = res.refresh_token;
    import_consola2.default.info(`Refresh token: \`${refreshToken}\``);
    entries.push(["CHROME_REFRESH_TOKEN", refreshToken]);
  }
  const publishTarget = await prompt(
  ...
  const submitForReview = await prompt(
...
async function initFirefox(previousOptions) {
  const entries = [];
  console.log();
  import_consola2.default.start("Firefox Addon Store\n");
...
async function initEdge(previousOptions) {
  const entries = [];
  console.log();
  import_consola2.default.start("Edge Addon Store\n");
  import_consola2.default.info(
  ...
  const accessTokenUrl = await prompt(
...
async function updateEnvFile(entries) {
  import_consola2.default.start(`Writing to \`${envFile}\`...`);
  let template = await (0, import_promises2.readFile)(envFile, "utf-8").catch(() => "");
...
${replacement}`;
  ...
  await (0, import_promises2.copyFile)(envFile, backupFilename).then(() => {
    import_consola2.default.info(`Backed up old \`${envFile}\` to \`${backupFilename}\``);
  }).catch(() => {
  });
  await (0, import_promises2.writeFile)(envFile, template, "utf-8");
  console.log();
  import_consola2.default.success("Wrote config to `.env.submit`");
}

// src/cli.ts
var import_consola3 = __toESM(require("consola"), 1);
var import_dotenv = require("dotenv");
(0, import_dotenv.config)({ path: ".env.submit" });
var cli = (0, import_cac.cac)("publish-extension");
cli.version(version);
cli.help();
cli.option(
  "--dry-run",
...
cli.command("", "Submit an extension to multiple stores for review").action(async (flags) => {
  const config2 = configFromFlags(flags);
  try {
    await submit(config2);
  } catch (err) {
